...
def main():
    ...
    if not ops.exists(input_image_path):
        print('input image path: {:s} not exists'.format(input_image_path))
        return
    insseg_cfg_path = args.insseg_cfg_path
    if not ops.exists(insseg_cfg_path):
        print('input innseg cfg path: {:s} not exists'.format(insseg_cfg_path))
        return
    ...
    print('Start to segment input image ...')
    ...
    print('save segment and cluster result into {:s}'.format(save_dir))
